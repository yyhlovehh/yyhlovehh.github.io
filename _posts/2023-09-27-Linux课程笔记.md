...
在系统中创建软链接，可以将文件、文件夹链接到其他位置，类似于windows系统中的快捷方式，只是一个指向。
...
查看ip：`ifconfig`，若无法使用，需要通过yum安装一个程序。
...
IP地址非常难记，有没有什么办法可以通过主机名或替代的字符地址去代替数字化的IP地址呢？实际上，我们一直通过字符化地址去访问服务器，很少指定IP地址。通常说的网址，即为域名。

流程：

![域名解析](https://raw.githubusercontent.com/yyhlovehh/yyhlovehh.github.io/master/202310071116531.png)
...
![固定IP](https://raw.githubusercontent.com/yyhlovehh/yyhlovehh.github.io/master/202310071140048.png)
...
![端口1](https://raw.githubusercontent.com/yyhlovehh/yyhlovehh.github.io/master/202310080356975.png)

![端口2](https://raw.githubusercontent.com/yyhlovehh/yyhlovehh.github.io/master/202310080357751.png)
...
功能：查看主机运行状态，类似于任务管理器，默认每隔5s刷新一次。

语法：`top`，查看基础信息

top内容：

![主机状态监控1](https://raw.githubusercontent.com/yyhlovehh/yyhlovehh.github.io/master/202310080422589.png)

![主机状态监控2](https://raw.githubusercontent.com/yyhlovehh/yyhlovehh.github.io/master/202310080429187.png)
...
修改文件、文件夹权限。r:读；w:写；x:对于文件夹为cd命令是否能够执行，对于文件则为是否可以执行。
...
chmod u=rwx,g=rx,o=x hellot.txt,将文件权限修改为rwxr-x--x。其中，u表示user所属用户权限，g表示group组权限，o表示other其他用户权限。
...
ctrl+C：强制停止命令或者删除正在输入的命令

ctrl+L：清屏或者clear

ctrl+D:退出当前账户的登录，即等于exit。或者退出某些特定程序的专属页面，eg:python

history:查看历史输入过的全部命令。可以与grep和管道符|配合使用

！:！命令前缀，自动执行上一次匹配前缀的命令

ctrl+r:输入命令匹配历史命令。配合键盘左右键得到命令
